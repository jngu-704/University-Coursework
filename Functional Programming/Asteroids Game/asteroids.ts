...
function asteroids() {
	...
	// create new laser
	function new_laser(x:number, y:number, deg:number, asteroids:Elem[]){
		let laser:Elem = new Elem(svg, "rect", g.elem)
			.attr("fill", "#FF0000")
	...
	// Create asteroids
	function create_asteroid(x:number, y:number, width:number, height:number, asteroids:Elem[],speed:number){
		let asteroid: Elem = new Elem(svg, "rect")
			.attr("fill", "#654321")
			.attr("x", x).attr("y", y)
			.attr('width', width).attr('height', height)
	
		// set a random direction
		let x_incre = get_randomnum(),
			y_incre = get_randomnum();
	...
	ship.observe<MouseEvent>('mousedown')

		.flatMap(({clientX, clientY}) => mousemove
	...
	function generate_asteroids(asteroids:Elem[]){
		...
		else if (asteroids.length < 4) create_asteroid(300,0,60,60,asteroids,10);
	...
	function move(x:number, y:number, r:number):string
	{
		let
			translate:string = "translate (".concat((x).toString(), " ", (y).toString(), ")"),
			rotate:string = " rotate (".concat(r.toString(), ")");
		return translate + rotate
	}


	// check if an object has collided with any asteroids
	function collision(obj:Elem, asteroids:Elem[]){
		asteroids.forEach((asteroid) => {
			let 
				x = Number(obj.attr('x')),
				y = Number(obj.attr('y')),
				x_coord = Number(asteroid.attr('x')),
				y_coord = Number(asteroid.attr('y')),
				width = Number(asteroid.attr('width')),
				height = Number(asteroid.attr('height'));
	...
	// check if an object has collided with any asteroids
	function ship_collision(obj:Elem, asteroids:Elem[], x:number, y:number){
		asteroids.forEach((asteroid) => {
			let 
				x_coord = Number(asteroid.attr('x')),
				y_coord = Number(asteroid.attr('y')),
